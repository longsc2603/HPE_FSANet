...
def main():
	...
	onlyfiles_mat = [f for f in listdir(mypath) if  f.endswith('.mat')]
	onlyfiles_jpg = [f for f in listdir(mypath) if f.endswith('.jpg')]
	...
	for i in tqdm(range(len(onlyfiles_jpg))):
		...
		mat_contents = sio.loadmat(mypath + '/' + mat_name) # loading file.mat
		pose_para = mat_contents['Pose_Para'][0] # to take row of matrix Pose_Para
		pt2d = mat_contents['pt2d']
		
		pt2d_x = pt2d[0,:] # to take first row and all column
		pt2d_y = pt2d[1,:] # to take second row and all column
		...
		if ((sum(pt2d_idx) != len(pt2d_idx)) | (sum(pt2d_idx) != len(pt2d_idx))):
			pt2d_id = pt2d_idx
			# It will will choose the length of matrix that has lengthe less than  to apply to remove elements in matrix pt2d_x and pt2d_y
