...
def main():
	...
	# print(randFlag)
	output_train_path = output_path+'_train4.npz'
	...
	for i in range(start,len(onlyfiles_png)+start):
		print('object %d' %i)
		...
		for j in tqdm(range(len(onlyfiles_png[i-start]))):
			
			img_name = onlyfiles_png[i-start][j]
			txt_name = onlyfiles_txt[i-start][j]
			...
			if len(detected) > 0:
				...
				for  d in detected:
					...
					xv = []
					yv = []
					for  value in d['keypoints'].values():
						xv.append(value[0]) 
						yv.append(value[1])
