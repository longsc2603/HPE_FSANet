...
class CapsuleLayer1d(nn.Module):
    def __init__(self, num_in_capsule,
                 in_capsule_dim, num_out_capsule,
                 out_capsule_dim, routings=3):
...
class FSANet(nn.Module):
    ...
    def forward(self, x, num_image=1):
...
# I couldn't use this class due to so many bugs that I don't know how to fix,
# so I decided to modify FSA-Net class and use the FSA-Net 3 times for
# 3 images of the triplet instead
"""
class TripletNet(nn.Module):
    ...
    def forward(self, x1, x2=None, x3=None, num_image=3):
        if num_image == 3:
            output1, V1 = self.embedding_net(x1)
            output2, V2 = self.embedding_net(x2)
            output3, V3 = self.embedding_net(x3)
        else:
            model = FSANet(var=False)
            output, _ = model(x1)
            return output
        
        return output1, output2, output3, V1, V2, V3
"""
...
class TotalLoss(nn.Module):
    ...
    """
    """
    Total Loss = Triplet Loss + Average MAE Loss of each of the three images
    """
    ...
    def TripletLoss(self, anchor, positive, negative, size_average=True):
        distance_positive = torch.sum(torch.pow((anchor - positive), 2))  # .pow(.5)
        distance_negative = torch.sum(torch.pow((anchor - negative), 2))  # .pow(.5)
        losses = F.relu(distance_positive - distance_negative + self.margin)

        return torch.mean(losses) if size_average else torch.sum(losses)

    def forward(self, anchor_features, positive_features,
                negative_features, pred_anchor, pred_positive,
                pred_negative, actual_poses, size_average=True):
        triplet_loss = self.TripletLoss(anchor_features, positive_features, negative_features, size_average)
        L1_loss_anchor = torch.mean(self.L1loss(pred_anchor, actual_poses[:, 0]), dim=0)
        L1_loss_positive = torch.mean(self.L1loss(pred_positive, actual_poses[:, 1]), dim=0)
        L1_loss_negative = torch.mean(self.L1loss(pred_negative, actual_poses[:, 2]), dim=0)

        return triplet_loss + L1_loss_anchor + L1_loss_positive + L1_loss_negative


if __name__ == '__main__':
    torch.random.manual_seed(10)
    model = FSANet(var=True)
    print('##############PyTorch################')
